...
const typeOptions = ['All types', 'Entire apartment', 'Entire cottage', 'Entire villa', 'Private room', 'Entire studio', 'Entire penthouse', 'Entire townhouse', 'Entire bungalow'];
...
export default function LocationPage() {
  ...
  const results = useMemo(() => {
    ...
    list = list.filter((a) => a.price <= maxPrice && a.bedrooms >= minBeds);
  ...
  }, [locationParam, selectedType, sortBy, maxPrice, minBeds]);
      ...
      <div className="location-page__inner">
        ...
        <aside className="location-filter" aria-label="Filters">
          ...
          <div className="location-filter__group">
            <label className="location-filter__label" htmlFor="type-select">Property type</label>
            <select
              id="type-select"
          ...
          <div className="location-filter__group">
            <label className="location-filter__label" htmlFor="price-range">
          ...
          <div className="location-filter__group">
            <label className="location-filter__label" htmlFor="beds">
          ...
          <div className="location-filter__group">
            <label className="location-filter__label" htmlFor="sort-select">Sort by</label>
            <select
              id="sort-select"
              className="location-filter__select"
              value={sortBy}
          ...
          <div className="location-filter__locations">
            <p className="location-filter__label">Browse by location</p>
            {locations.map((loc) => (
              <button
                key={loc}
                className={`location-filter__loc-btn ${locationParam === loc ? 'location-filter__loc-btn--active' : ''}`}
                onClick={() => navigate(`/locations?location=${encodeURIComponent(loc)}`)}
              >
                {loc}
              </button>
            ))}
        ...
        <main className="location-results">
          <div className="location-results__heading">
            ...
            <p className="location-results__count">
              {results.length} accommodation{results.length !== 1 ? 's' : ''} found
            </p>
          </div>
          ...
          {results.length === 0 ? (
            ...
            <ul className="location-cards" aria-label="Accommodation listings">
              {results.map((acc) => (
                <li key={acc._id}>
                  <button
                    className="location-card"
                    onClick={() => navigate(`/locations/${acc._id}`)}
                  ...
                  >
                    ...
                    <div className="location-card__body">
                      <div className="location-card__top">
                        <span className="location-card__type">{acc.type}</span>
                        <span className="location-card__rating">
                          <StarRating rating={acc.rating} />
                          <span>{acc.rating} ({acc.reviews})</span>
                        </span>
                      </div>
                      <h2 className="location-card__title">{acc.title}</h2>
                      ...
                      <ul className="location-card__amenities">
                        ...
                        {acc.amenities.length > 4 && <li>+{acc.amenities.length - 4} more</li>}
                      </ul>
                      <div className="location-card__footer">
                        <div className="location-card__details">
                          ...
                          <span>·</span>
                          <span>Up to {acc.guests} guests</span>
                        </div>
